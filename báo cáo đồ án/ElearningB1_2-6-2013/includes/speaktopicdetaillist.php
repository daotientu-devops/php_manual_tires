<?php
if(isset($_GET['speaktopdetailid']))
{
	$speaktopdetailid=$_GET['speaktopdetailid'];
	$strSQL = "SELECT * FROM lessons_speaktopic WHERE catid='".$_GET['speaktopdetailid']."' AND state=1";
...
{
	header('Location: view_speaktopcatlist.php');
}
?>
<div class="heading">
<div class="title">
...
<ul>
	...
	$strSQL = "SELECT * FROM lessons_speaktopic WHERE catid = '".$_GET['speaktopdetailid']."' AND state=1 LIMIT 0,1";
	$result = mysql_query($strSQL, $link);
	$sort=0;
	while($row=mysql_fetch_array($result))
	{
		...
		echo 'Phần 2 (5 phút): Thí sinh trình bày chủ đề đã bốc thăm. Phần trình bày phải có bố cục rõ ràng, giới thiệu nội dung, phát triển ý, kết luận, biết sử dụng phương tiện liên kết ý. Tránh liệt kê hàng loạt mà không phát triển kỹ từng ý.<br/><br/>';
		echo 
		'<div style="width: 618px; text-align: left;">'.$row['topic_name'].'</div>'
		;
		echo '<br/>';
		echo 'Bài nói mẫu:';
		echo '<div style="width: 618px; text-align: left;">'.$row['sample'].'</div>'
		;
...
if(isset($_POST['submit']))
{
	...
	else
	{
		...
		elseif($_SESSION['state']==1)
		{
			...
			if($row2>0)
			{
				$save_path="CMSforE/uploaded/lessons_speaktopic/"; 
			...
			if($file['size']>0)
			{
			$strSQL4="UPDATE userfile SET speaktopid='".$speaktopdetailid."',speaktopfile='".$save_path.$file['name']."' WHERE userid='".$_SESSION['logUser']."'";
			...
			if($file['size']>0)
			{
			$strSQL3="INSERT INTO userfile(userid, speaktopid, speaktopfile) VALUES('".$_SESSION['logUser']."','".$speaktopdetailid."','".$save_path.$file['name']."')";
