...
{
	$strSQL = "SELECT * FROM lessons_readtest WHERE catid='".$_GET['readtestdetailid']."' AND state=1";
...
<ul>
	...
	$strSQL = "SELECT * FROM lessons_readtest WHERE catid = '".$_GET['readtestdetailid']."' AND state=1 LIMIT 0,10";
	$result = mysql_query($strSQL, $link);
	$sort=0;
	while($row=mysql_fetch_array($result))
	{
		...
		'Answer is: '.
		'
		<select name="txtanswer'.$sort.'" id="txtanswer">
		<option value=""></option>
