...
<div class="userleft">
    ...
    <td align="left">
	<input type="text" name="username" id="username" value="" onblur="return check_username();" style="width: 200px; border: 1px solid #790000;" />
	</td>
	<td>
		<div id="Info"></div>
		<span id="Loading"><img src="./images/icon/loader.gif" alt="" /></span>
	</td>
    </tr>
   	<tr>
    ...
    <td align="left"><input type="text" name="email" id="email" style="width: 200px; border: 1px solid #790000;" /></td>
    </tr>
    ...
    <tr>
    <td align="right"><div style="font-weight: bold; color: #790000">Nơi công tác: <font color="#FF0000">*</font></div></td>
    ...
    else
    {
        ...
        else
        {
            ...
            if(mysql_num_rows(@$results) > 0) // not available
            {
                ...
                $save_path="CMSforE/uploaded/avatar/"; 
                $file = $_FILES['userfile']; 
                $type = $_FILES['userfile']['type'];
                
                if(isset($_FILES['userfile']) && $file['size']>0 ){
                    
                    if($file['size'] < 524000 ){

                        if($type == 'image/png' || $type == 'image/gif' || $type == 'image/jpg' || $type == 'image/jpeg'){
							//thiếu đường dẫn save_path
                            ...
                            $sql = "INSERT INTO users(username, password, name, date, month, year, sex, address, country, place_of_work, tel, email, RoleID)
                            values ('".$_POST['username']."','".md5($_POST['password'])."','".$_POST['name']."','".$_POST['date']."','".$_POST['month']."','".$_POST['year']."','".$_POST['sex']."','".$_POST['address']."','".$_POST['country']."','".$_POST['place_of_work']."','".$_POST['tel']."','".$_POST['email']."','students')
                            ";
                            }
                            ...
                            echo 'file upload không đúng định dạng.';
                        }

                    }else{
                        echo 'file upload quá dung lượng cho phép (500kb).';
                    }
                }  else {
                    $sql = "INSERT INTO users(username, password, name, date, month, year, sex, address, country, place_of_work, tel, email, RoleID)
                            values ('".$_POST['username']."','".md5($_POST['password'])."','".$_POST['name']."','".$_POST['date']."','".$_POST['month']."','".$_POST['year']."','".$_POST['sex']."','".$_POST['address']."','".$_POST['country']."','".$_POST['place_of_work']."','".$_POST['tel']."','".$_POST['email']."','Student')
                            ";
