<?php
if(isset($_GET['writeansdetailid']))
{
	$writeansdetailid=$_GET['writeansdetailid'];
	$strSQL = "SELECT * FROM lessons_writeanswer WHERE catid='".$_GET['writeansdetailid']."' AND state=1";
...
{
	header('Location: view_writeanscatlist.php');
}
?>
<div class="heading">
<div class="title">
...
<ul>
	...
	$strSQL = "SELECT * FROM lessons_writeanswer WHERE catid = '".$_GET['writeansdetailid']."' AND state=1 LIMIT 0,1";
	$result = mysql_query($strSQL, $link);
	$sort=0;
	while($row=mysql_fetch_array($result))
	{
		$sort++;
		echo 
		'<div style="width: 618px; text-align: left;">'.$row['content_question'].'</div>'
		;
		//echo '<br/>';
		//echo 
		...
		echo '<b>Example:</b>';
		echo 
		'<div style="width: 618px; text-align: left;">'.$row['content_answer'].'</div>'
		;
	}
	?>
...
if(isset($_POST['submit']))
{
	...
	else
	{
		...
		elseif($_SESSION['state']==1)
		{
			...
			if($row2>0)
			{
				$save_path="CMSforE/uploaded/lessons_writeanswer/"; 
			...
			if($file['size'] > 0)
			{
			$strSQL4="UPDATE userfile SET writeansid='".$writeansdetailid."',writeansfile='".$save_path.$file['name']."' WHERE userid='".$_SESSION['logUser']."'";
			...
			{
			$save_path="uploaded/lessons_writeanswer/"; 
			$file = $_FILES['userfile']; 
			$type = $_FILES['userfile']['type'];
			if($type == 'application/msword' || $type == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')
			{
			...
			if($file['size'] > 0)
			{
			$strSQL3="INSERT INTO userfile(userid, writeansid, writeansfile) VALUES('".$_SESSION['logUser']."','".$writeansdetailid."','".$save_path.$file['name']."')";
